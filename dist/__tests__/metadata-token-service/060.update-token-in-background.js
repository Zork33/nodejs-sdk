...
describe('MetadataTokenService.getToken', () => {
    ...
    it('the first time the token is requested immediately when the service is created', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        expect(testLoggerFn.mock.calls)
            ...
            [
                'debug',
                "metadata-token.ctor: url: %s, doUpdateTokenInBackground: %s, axiosOptions: %o" /* debug_ctor */,
                metadata_token_service_consts_1.DEFAULT_URL,
                true,
                metadata_token_service_consts_1.DEFAULT_OPTIONS,
            ],
            [
                'trace',
                "metadata-token.getToken" /* trace_getToken */,
            ],
            [
                'trace',
                "metadata-token._getToken" /* trace__getToken */,
            ],
            [
                'trace',
                "metadata-token.initialize" /* trace_initialize */,
            ],
            [
                'trace',
                "metadata-token.fetchToken" /* trace_fetchToken */,
            ],
            [
                'trace',
                "metadata-token.setIamResponse" /* trace_setIamResponse */,
            ],
            [
                'debug',
                "new token. TTL: %s%s" /* debug_new_token_was_received */,
                new hr_interval_1.HRInterval(TTL * 1000),
                '',
            ],
            [
                'trace',
                "metadata-token.setTimer" /* trace_setTimer */,
            ],
        ]);
        yield metadataTokenService.destroy();
    }));
    ...
    it('dispose stops the timer and getToken() starts to throw an error', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        expect(yield metadataTokenService.getToken())
            .toBe('123');
        yield expect(metadataTokenService.destroy())
            .resolves
            .toBeUndefined();
        yield expect(metadataTokenService.getToken())
            .rejects
            .toThrow(new Error("service is destroyed" /* err_service_is_destroyed */));
        yield expect(metadataTokenService.destroy())
            .rejects
            .toThrow(new Error("service is destroyed" /* err_service_is_destroyed */));
    }));
