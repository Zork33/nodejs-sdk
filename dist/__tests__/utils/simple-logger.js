    ...
    for (const env of [false, true, 'AAA']) {
        for (const conf of [
            ...
            it(`general: ${env}, ${conf.level}`, () => {
                ...
                switch (env) {
                    case false:
                        logger = conf.level === undefined ? new simple_logger_1.SimpleLogger() : new simple_logger_1.SimpleLogger({ level: conf.level });
                        break;
                    case true:
                        process.env.LOG_LEVEL = conf.level;
                        logger = new simple_logger_1.SimpleLogger();
                        break;
                    default:
                        process.env[env] = conf.level;
                        logger = new simple_logger_1.SimpleLogger({ envKey: env });
